package crudproductos.services.utils
  ...
  implicit def fromObjectCross[T: Marshaller](origin: String)(obj: T) =
    new CompletionMagnet {
      def route: StandardRoute = new CompletionRoute(OK,
        RawHeader("Access-Control-Allow-Origin", origin) :: RawHeader("Access-Control-Allow-Headers", "Content-Type"):: RawHeader("Access-Control-Allow-Methods", "GET, POST, OPTIONS,PUT,DELETE") :: RawHeader("Content-Type", "application/json,text/plain") :: Nil, obj)
    }
